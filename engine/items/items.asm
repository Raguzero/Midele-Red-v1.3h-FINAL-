...
ItemUsePtrTable:
	...
	dw ItemUseEvoStone      ; ICE_STONE  ; NUEVO OBJETO  ANTES ??? PP_UP BUG
	...
	dw UnusableItem
	dw UnusableItem
	dw ItemUseVitamin  ; PK_HEX  NUEVO OBJETO
	dw UnusableItem  ; MUSIC PLAYER  NUEVO OBJETO
...
; Determine BallFactor. It's 8 for Great Balls and 12 for the others.
	ld a, [wcf91]
	; NUEVO PARA MEJORAR LA SAFARI BALL (30% ATRAPAR) Y QUITAR BUG DE SUPERBALL QUE ERA MEJOR CON PS MAXIMO QUE LA ULTRA BALL
	;cp GREAT_BALL
	cp SAFARI_BALL
	; NUEVO PARA MEJORAR LA SAFARI BALL (30% ATRAPAR) Y QUITAR BUG DE SUPERBALL QUE ERA MEJOR CON PS MAXIMO QUE LA ULTRA BALL
	ld a, 12
	jr nz, .skip1
	ld a, 4  	; NUEVO PARA MEJORAR LA SAFARI BALL (30% ATRAPAR) Y QUITAR BUG DE SUPERBALL QUE ERA MEJOR CON PS MAXIMO QUE LA ULTRA BALL
...
.printTransferredToPCText
	call PrintText
	; NUEVO PARA AVISAR DE CAJA LLENA
...
; NUEVO PARA MICOMON, DARKPIKA Y CATERMANO
  ...
  add hl, bc  ; ahora hl apunta a la especie del poke
  ld a, [hl]
  cp DARK_PIKA
  jr z, .especial
  cp MICOMON
...
; NUEVO OBJETO
	jp .end_mgene
;joenote - added code for the PK_HEX
.useMGene
    push hl
    ld bc, wPartyMon1HPExp - wPartyMon1 
    add hl, bc ; ahora hl apunta al esfuerzo en PS
;DVS AL MAXIMO
   ld a, $ff
    ld [hli], a
    ld [hli], a ; PS al máximo
    ld [hli], a
    ld [hli], a ; Ataque al máximo
    ld [hli], a
    ld [hli], a ; Defensa al máximo
    ld [hli], a
    ld [hli], a ; Velocidad al máximo
    ld [hli], a
    ld [hli], a ; Especial al máximo
    ld [hli], a
    ld [hl], a ; DVs al máximo
    ld bc, wPartyMon1Level - wPartyMon1PP + 1
    add hl, bc ; hl now points to level
    ld a, [hl] ; a = level
; NUEVO PARA MICOMON, DARKPIKA Y CATERMANO
    ;cp MAX_LEVEL
...
.especialPKHEX
  ...
  pop af
  cp MAX_LEVEL_ESPECIAL
  ld a, MAX_LEVEL_ESPECIAL
.topePuestoPKHEX
; NUEVO PARA MICOMON, DARKPIKA Y CATERMANO
  jr z, .noSubeDeNivel ; can't raise level above 100
  ld [hl], a ; guarda el nivel máximo
  ld [wCurEnemyLVL], a
  push hl
  push de
  ld d, a
...
.noSnorlaxToWakeUp
	...
	jp PrintText
.inBattle
; NUEVO PARA QUE NO FUNCIONA POKEFLUTE EN BATALLA
	;xor a
...
.RandomLoop
	...
	jr c, .SetBite
	and %11
	cp 5 ;cp 2 	; NUEVO PARA MEJORAR GOOD_ROD
	jr nc, .RandomLoop
	; choose which monster appears
	ld hl, GoodRodMons
	add a
...
ThrowBallAtTrainerMon:
	...
	call PrintText
	; NUEVO PARA EVITAR GASTAR UNA BALL SI LO USAS CONTRA ENTRENADOR
	;jr RemoveUsedItem
	ret
	; NUEVO PARA EVITAR GASTAR UNA BALL SI LO USAS CONTRA ENTRENADOR
