...
; Attacks that aren't finished after they faint the opponent.
	...
	db TWINEEDLE_EFFECT
	db RAGE_EFFECT
; NUEVO PARA CORREGIR BUG DE NO RECARGAR AL MATAR CON HYPER BEAM
...
MainInBattleLoop:
; NUEVO PARA CORREGIR BUG TOXIC + LEECH SEED DAÑO DOBLE
...
; NUEVO PARA CORREGIR BUG TOXIC + LEECH SEED DAÑO DOBLE
	...
	xor a
	ld [wFirstMonsNotOutYet], a
	call SelectEnemyMove ; NUEVO AÑADIDO PARA QUE LA IA NO PREDIGA SIEMPRE AL CAMBIAR
...
.selectEnemyMove
; NUEVO PARA NO HACER FEO EL CODIGO PARA CORREGIR BUG PP DE WRAP
  ld a, [wEnemyBattleStatus1]
  and (1 << USING_TRAPPING_MOVE)
  jr z, .noReponerWrap
  ; hacemos que la IA use el ataque en el que está atrapada
  ld a, [wEnemyMoveListIndexPrevio]
...
.noReponerWrap
; NUEVO PARA NO HACER FEO EL CODIGO PARA CORREGIR BUG PP DE WRAP
	;call SelectEnemyMove ; NUEVO COMENTADO PARA QUE LA IA NO PREDIGA SIEMPRE AL CAMBIAR
	ld a, [wLinkState]
	cp LINK_STATE_BATTLING
	jr nz, .noLinkBattle
; link battle
...
HandlePlayerBlackOut:
	...
	jr z, .sony1Battle
	cp MAROWAK
    jr nc, .alCarrer ; lo pokes salvajes especiales no tienen mensaje de derrota
	cp OPP_RAGU
    jr c, .alCarrer; salta si no es un oponente personalizado
    ld hl, TrainerEndBattleText
	jr .mensajeDerrota
.sony1Battle
    ld hl, Sony1WinText
.mensajeDerrota
    call BorrarPokeYMostrarMensaje
...
; link battle
	...
	add hl, bc
	ld a, [hl]
	jp .done ; antes jr , NUEVO CAMBIADO PARA FORZAR STRUGGLE EN ENEMIGOS
.noLinkBattle
	ld a, [wEnemyBattleStatus2]
...
; NUEVO PARA QUE SUEÑO SEA COMO GEN 2
...
.unableToSelectMove
	ld a, $ff
	jp .done ; NUEVO PARA FORZAR STRUGGLE EN ENEMIGOS
.canSelectMove
; NUEVO PARA NO HACER FEO EL CODIGO PARA CORREGIR BUG PP DE WRAP
  ld a, [wEnemyBattleStatus1]
  and (1 << USING_TRAPPING_MOVE)
  jr z, .saltarWrap
  ; Está usando wrap: dejamos elegir a la IA pero guardamos lo que debe hacer si sigue atrapada
  ld a, [wEnemyMoveListIndex]
  ld [wEnemyMoveListIndexPrevio], a
.saltarWrap
; NUEVO PARA NO HACER FEO EL CODIGO PARA CORREGIR BUG PP DE WRAP
; NUEVO PARA FORZAR STRUGGLE EN ENEMIGOS
	...
	;jr nz, .done
	    ; comprobamos si el número de movimientos elegibles por la CPU es 0 o mayor
    ; Si es 0, usará Struggle. Si es 1, podrá elegir
    ld a, [wEnemyDisabledMove]
    and a
    ld hl, wEnemyMonPP
    jr nz, .handleDisabledMove
    ...
    and $3f
    ld a, STRUGGLE
    jr z, .done    ; si no hay PP, usa Struggle
	jr .atLeastTwoMovesAvailable
.handleDisabledMove
    swap a
    and $f ; get disabled move
    ld b, a
...
.allMovesChecked
    and $3f ; any PP left?
    ld a, STRUGGLE
    jr z, .done ; si no, usa Struggle
; NUEVO PARA FORZAR STRUGGLE EN ENEMIGOS
.atLeastTwoMovesAvailable
    ld hl, wEnemyMonMoves ; NUEVO PARA FORZAR STRUGGLE EN ENEMIGOS
	ld a, [wIsInBattle]
	dec a
...
; NUEVO PARA FORZAR STRUGGLE EN ENEMIGOS
    ...
    ld c, a
    add hl, bc
    ld a, [hl] ; lee los PP del movimiento
    and $3f    ; mira si son 0
    ld a, d
    pop bc
    pop de
    pop hl
    jr z, .chooseRandomMove ; no tiene PP, a escoger otra vez
; NUEVO PARA FORZAR STRUGGLE EN ENEMIGOS
.done
	ld [wEnemySelectedMove], a
	ret
...
ExecutePlayerMove:
	...
	ld [wMoveDidntMiss], a
	;ld a, $a ; NUEVO borrado PARA CORREGIR EFECTIVIDAD DE DUAL TIPOS
	ld [wDamageMultipliers], a
	ld a, [wActionResultOrTookBattleTurn]
	and a ; has the player already used the turn (e.g. by using an item, trying to run or switching pokemon)
...
; NUEVO PARA CORREGIR EL BUG DE FLY/DIG INVULNERABLE SI SUFRE EFECTO PARALISIS 
...
.playerTurn
	ld [hl], a
	; NUEVO PARA QUITAR GRAMMAR JAPONES INNECESARIO
	;ld [wd11e], a
...
.gotTextToPrint
	...
	; NUEVO BORRADO PARA CORREGIR JUMP KICK EFFECT  
	;ld hl, wDamage ; since the move missed, wDamage will always contain 0 at this point.
	 	; NUEVO BORRADO PARA CORREGIR JUMP KICK EFFECT             
				  ; Thus, recoil damage will always be equal to 1
	                ; even if it was intended to be potential damage/8.
; NUEVO PARA CORREGIR JUMP KICK EFFECT 
  ld a, [H_WHOSETURN]
    and a
    ld hl, wEnemyMonMaxHP
    jr nz, .pselegidos ; Si falla el enemigo, hl ya apunta a los PS del enemigo
    ; En caso contrario falla el jugador, por lo que usamos los PS máximos del jugador en lugar de los del enemigo
    ld hl, wBattleMonMaxHP
.pselegidos
; NUEVO PARA CORREGIR JUMP KICK EFFECT 
	ld a, [hli]
	...
	srl a
	rr b
; NUEVO PARA JUMP KICK SEA 50%HP COMO GEN ACTUAL Y PARA CORREGIR AÑADIENDO wdamage
	;srl a
	;rr b
	;srl a
	;rr b
...
; NUEVO PARA HOLA REINA
    cp 1
    jr nz, .finReina ; Solo Hola Reina tiene potencia 1 sin ser de daño especial
    ld a, [wBattleMonLevel]
    add 50
    cp 150
    jr c, .ciencomomucho
...
; bc /= 4 (scale enemy's defensive stat)
	...
	srl b
	rr c
; NUEVO CORREGIDO BUG QUE CONGELABA EL JUEGO SI USABAS 6 LEER (3 SCREECH o parecido) para bajarle la defensa a pokes con nivel muy bajo y penosa defensa
	ld a, c
...
.next
	...
	ld e, a ; e = level
	ld a, [wCriticalHitOrOHKO]
; NUEVO BORRADO PARA CORREGIR BUG DE DAÑO CRITICO EN POKEMON APARTIR DE NV128(NO SE CUAL NV), AHORA NO SE MULTIPLICA EL DAÑO X2 CUANDO SE LEE SINO CUANDO SE VA A USAR LA FORMULA DAÑO
	;and a ; check for critical hit
	;jr z, .done
...
.nc
	inc hl
	ldi [hl], a
; NUEVO AÑADIDO PARA CORREGIR BUG DE DAÑO CRITICO EN POKEMON APARTIR DE NV128(NO SE CUAL NV), AHORA NO SE MULTIPLICA EL DAÑO X2 CUANDO SE LEE SINO CUANDO SE VA A USAR LA FORMULA DAÑO
  ; Si es crítico, el nivel se multiplica por 2
    ld a, [wCriticalHitOrOHKO]
    and a ; mira si es crítico
    jr z, .nocrit
    dec hl
    ld a, [hl]
    add a
...
CriticalHitTest:
	xor a
	ld [wCriticalHitOrOHKO], a
; NUEVO PARA QUE GOLPE CRITICO SEA COMO EN GSC
;	ld a, [H_WHOSETURN]
;	and a
;	ld a, [wEnemyMonSpecies]
;	jr nz, .handleEnemy
...
;.handleEnemy
	...
	;ld a, [wMonHBaseSpeed]
	;ld b, a
; AHORA SI NUEVO ARREGLADO BUG DE FOCUS ENERGY QUE BAJA % CRITICO
...
;.focusEnergyUsed
;	sla b ;srl b  ; NUEVO ARREGLADO BUG DE FOCUS ENERGY QUE BAJA % DE CRITICO
;.noFocusEnergyUsed
; AHORA SI NUEVO ARREGLADO BUG DE FOCUS ENERGY QUE BAJA % CRITICO
  bit GETTING_PUMPED, a         ; mira si usa Foco Energía
    jr z, .noFocusEnergyUsed
    sla b                        ; velocidad base * 2
    jr nc, .skipNoFocusEnergyUsed
    ld b, $ff                    ; trunca en 255/256
    jr .skipNoFocusEnergyUsed
.noFocusEnergyUsed
    srl b                        ; velocidad base / 2
.skipNoFocusEnergyUsed
; AHORA SI NUEVO ARREGLADO BUG DE FOCUS ENERGY QUE BAJA % CRITICO
...
; AHORA SI NUEVO ARREGLADO BUG DE FOCUS ENERGY QUE BAJA % CRITICO
	...
	ld b, $ff
.SkipHighCritical
; NUEVO PARA CRITICOS COMO EN GEN 2 GSC
  ld a, 17
  cp b
  jr c, .compruebaCritico
  ld b, a
.compruebaCritico
; NUEVO PARA CRITICOS COMO EN GEN 2 GSC
...
; check if the move the target last selected was Normal or Fighting type
	...
	cp FIGHTING
	jr z, .counterableType
; NUEVO PARA STRUGGLE UNKNOWN TYPE
    cp UNK
	jr z, .counterableType
; NUEVO PARA STRUGGLE UNKNOWN TYPE
...
ApplyDamageToEnemyPokemon:
	...
	;  NUEVO PARA CORREGIR BUG DE DAÑO CONFUSION EN SUBSTITUTE
...
.loop
	call BattleRandom
; NUEVO PARA CORREGIR QUE PSYWAVE NO HAGA 0 EN OCASIONES
;;;joenote - add these two lines to make enemy psywave match [1, b)
	and a
	jr z, .loop
; NUEVO PARRA CORREGIR QUE PSYWAVE NO HAGA 0 EN OCASIONES
	cp b
	jr nc, .loop
	ld b, a
.storeDamage
...
.nullifyEffect
; NUEVO PARA CORREGIR QUE HYPER BEAM, ATAQUES EXPLOSIVOS Y ATAQUES RECOIL HAGAN RECARGAR, SUFRIR RECOIL, O DEBILITARSE AL GOLPEAR SUSTITUTOS
...
.pickMoveLoop
	...
	cp METRONOME
	jr z, .pickMoveLoop
	; NUEVO PARA QUE NO SALGA EN METRONOME
	cp MIDELE_POWER
    jr z, .pickMoveLoop
	cp DARK_THUNDER
    jr z, .pickMoveLoop
...
.loop
	...
	cp $ff
	jr z, StoreDamage ;jr z, .done ; NUEVO PARA CORREGIR EFECTIVIDAD DE DUAL TIPOS
...
; if the move type matches the "attacking type" and one of the defender's types matches the "defending type"
	...
	push bc
	inc hl
; NUEVO borrado PARA CORREGIR EFECTIVIDAD DE DUAL TIPOS
	;ld a, [wDamageMultipliers]
	;and $80
	;ld b, a
; NUEVO borrado PARA CORREGIR EFECTIVIDAD DE DUAL TIPOS
...
; NUEVO PARA CORREGIR EFECTIVIDAD DE DUAL TIPOS
	call Multiply
; divide by 10  ; NUEVO PARA CORREGIR EFECTIVIDAD DE DUAL TIPOS
	ld a, 10
	ld [H_DIVISOR], a
	ld b, $04
	call Divide
...
; NUEVO CORREGIDO BUG DE ESTADOS ALTERADOS Y LEECH SEED EN UN SUBSTITUTE
	ld a, [de] ;joenote - reload the effect info so that drain moves & dreameater miss substitutes
; NUEVO CORREGIDO BUG DE ESTADOS ALTERADOS Y LEECH SEED EN UN SUBSTITUTE
; NUEVO PARA QUE NO FALLE ATAQUES COMO DREAM EATER Y LEECH LIFE A UN SUSTITUTO POR ARREGLAR EL BUG DE LEECH SEED ANTE SUBSTITUTE
	;cp DRAIN_HP_EFFECT
	;jp z, .moveMissed
	;cp DREAM_EATER_EFFECT
	;jp z, .moveMissed
...
LoadEnemyMonData:
	...
	cp $2 ; is it a trainer battle?
	 ; NUEVO PARA CAMBIAR DVS POR DEFECTO DE ENTRENADORES
...
; NUEVO REGALO SHINY
    ; Si está forzado a tener IVs a tope (y por ello sere shiny), hacemos tal cosa. Si no, IVs al azar
    CheckEventHL EVENTO_FORZAR_15_IVS
    ld a, $ff
    ld b, $ff
    jr nz, .storeDVs
; NUEVO REGALO SHINY
...
.storeDVs
	...
	;dec a
    cp 2 ; mira si es un combate contra entrenador
  ; NUEVO PARA ARREGLAR BUG DE HP DE POKES REGALADOS POR EL CAMBIO DE LOS EVS/IVS DE ENTRENADOR
	  jr c, .not_use_stat_exp ; si no es combate (0) o es contra poke salvaje(1), no se usa la experiencia; AHORRO DE ESPACIO --> jr z, .not_use_stat_exp
...
.stat_exp_calc ; NUEVO PARA ARREGLAR BUG DE HP DE POKES REGALADOS POR EL CAMBIO DE LOS EVS/IVS DE ENTRENADOR
	...
	; NUEVO PARA ARREGLAR BUG DE HP DE POKES REGALADOS POR EL CAMBIO DE LOS EVS/IVS DE ENTRENADOR
	;dec a
    cp 2 ; mira si es contra entrenador
	; NUEVO PARA ARREGLAR BUG DE HP DE POKES REGALADOS POR EL CAMBIO DE LOS EVS/IVS DE ENTRENADOR
	 jr c, .calc_stats  ; NUEVO PARA AHORRO CODIGO --> jr z, .calc_stats
	; NUEVO PARA ENTRENADORES MAX EVS
	push bc
	push de
...
.loadMovePPs
	...
	ld de, wEnemyMonPP - 1
	predef LoadMovePPs
; NUEVO PARA ENEMIGOS MAX PP
   ld a, [wIsInBattle]
    cp $2 ; mira si es contra un entrenador
    jr nz, .finPP ; si no, no se le aplica subida de PP
    ; si sí, sí se le aplica
    ld hl, wEnemyMonPP
    ld d, NUM_MOVES
.subePPLoop
    ld a, [hl] ; leemos los PP del mov actual
    cp 40 ; si es un mov de 40 PP, se trata distinto (sus PP máximos son 61 y no 64)
    jr nz, .movNormal
    ld a, %11000000 + 61
    jr .ponePP
.movNormal
	cp 1
    jr z, .ponePP ; si es 1 es Esquema y en este no hay cambio de PP
    and a
    jr z, .finPP ; si es 0 es que no había movimiento: terminamos
    ld [H_DIVIDEND+3], a
    xor a
    ld [H_DIVIDEND], a
    ld [H_DIVIDEND + 1], a
    ...
    call Divide
    ld a, [H_QUOTIENT + 3]
    ; Chapuza suprema para multiplicar por 8
    rla
    rla
    rla
    and %11111000
    or %11000000 ; marca los PP como subidos del todo
.ponePP
    ld [hli], a
    dec d
    jr nz, .subePPLoop ; si no hemos hecho esto cuatro veces, no hemos terminado: repetimos con el siguiente
.finPP
; NUEVO PARA ENEMIGOS MAX PP
	ld hl, wMonHBaseStats
	ld de, wEnemyMonBaseStats
...
; NUEVO PARA SHINY
	...
	;sub 200 
	; NUEVO BORRADO PARA ENTRENADORES QUE NO EMPIECEN DESDE 200	
	;jp c, InitWildBattle 	; NUEVO PARA ENTRENADORES NO SON POKEMON
	ld [wTrainerClass], a
...
; NUEVO PARA SPRITES ENTRENADORES EN CUALQUIER BANK
;	ld a, [wLinkState]
;	and a
...
; been loaded with GetMonHeader.
	...
	; NUEVO PARA SPRITEBACK DE GSC USABLE
	;predef ScaleSpriteByTwo
 ld a, [wcf91]
   cp DARK_PIKA
  jr z, .gasol
	...
	cp DEX_ALTARIA
	jr z, .messi
	;[aquí, por cada poke que se vea enano tras Cofagrigus, un cp DEX_ESEPOKE jr z, .messi]
    cp DEX_CATERMANO
    jr c, .gasol
    cp DEX_HOPPIP
    jr c, .messi
...
; NUEVO PARA STEEL WING Y METAL CLAW
...
; NUEVO PARA CORREGIR BURN/PARALYSIS STACK
...
.statModifierDownEffect
	...
	jr c, .nonSideEffect
	call BattleRandom
; NUEVO PARA QUE LOS EFECTOS DE BAJADA DE STATS (ACID, BUBBLEBEAM, PSYCHIC, ETC.) SEAN DEL 10% EN VEZ DE 33.2%
...
.pickMoveToDisable
	...
	ld hl, wBattleMonPP
	jr nz, .enemyTurn
; NUEVO BORRADO PARA ARREGLAR DISABLE QUE NO SIRVA SI LOS ATAQUES DEL ENEMIGO NO TIENEN PP
	;ld a, [wLinkState]
	;cp LINK_STATE_BATTLING
