...
.healEffect
	...
	inc de
	inc hl
	jr nz, .fine ; NUEVO PARA ARREGLAR BUG DE 255 O 511 PS AL CURAR
	ld a, [de]
	sbc [hl]
	jp z, .failed ; no effect if user's HP is already at its maximum
.fine ; NUEVO PARA ARREGLAR BUG DE 255 O 511 PS AL CURAR
	ld a, b
	cp REST
	jr nz, .healHP
	push hl
...
; NUEVO PARA QUE SUEÑO SEA COMO GEN 2
	ld [hl], 3 ; clear status and set number of turns asleep to 2  ANTES 2
; NUEVO PARA QUE SUEÑO SEA COMO GEN 2
